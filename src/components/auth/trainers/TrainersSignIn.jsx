...
const TrainersSignIn = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const userDoc = await getDoc(doc(db, "trainers", user.uid));

      if (userDoc.exists()) {
        const userData = userDoc.data();

        if (userData.valid === true) {
          console.log("User is verified:", userData);

          sessionStorage.setItem("userUid", user.uid);

          navigate("/trainers-dashboard/profile");
        } else {
          setError(
            "Your account is not verified by the master admin. Please contact support."
          );
        }
      } else {
